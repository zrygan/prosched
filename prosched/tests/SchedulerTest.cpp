...
// ─── PID allocation ──────────────────────────────────────────────────────────
//
// GenerateProcessesCycle claims a pid on EVERY tick:
//
//     void GenerateProcessesCycle(int cpuCycles) {
//       int pid = nextPID++;                                   // <-- unconditional
//       if (generatingProcesses && cpuCycles % freq == 0) {
//         Process *p = generateProcess(&ctx, pid, cpuCycles);
//         std::lock_guard<std::mutex> lock(schedulerMutex);     // lock starts HERE
//         ...
//         nextPID++;                                           // <-- and again
//       }
//     }
//
// Two defects in one line, both fixed by moving the claim inside the `if` and
// under the lock:
//
// 1. It is OUTSIDE schedulerMutex, while CreateNamedProcess claims its pid
//    INSIDE it (Scheduler.h:443). ThreadSanitizer reports exactly this pair:
//    write in SchedulerLoop vs read in CreateNamedProcess. Two processes can
//    therefore receive the SAME pid — and PagingManager keys page tables,
//    interpreters and the backing store by pid, so they would share an address
//    space.
// 2. It burns a pid every tick even when nothing is generated, so pid numbers
//    are unrelated to how many processes exist. MEASURED: with generation OFF
//    for 300 ms, two consecutive `screen -s` processes got pid 1 and pid 5824.
//    Generated process NAMES come from the same counter (Scheduler.h:372), so
//    screen -ls shows names like "process5824" for the second process created.
//
// This test pins defect 2, which is deterministic. Defect 1 is the same line and
// is covered by the TSan probes (SchedulerConcurrencyProbe / SchedulerRngRaceProbe).
namespace SchedulerPidAllocation {

TEST(SchedulerPidAllocation, PidsAreNotConsumedWhileIdle) {
  AlgoContext ctx = makeSmallCtx("rr", 2, 5); // batch freq is 1e6: no generation
  prosched::PagingManager pm(64, 4096);
  prosched::Scheduler sched(ctx, &pm);
  sched.Start();

  prosched::Process *first = sched.CreateNamedProcess("first", 256);
  ASSERT_NE(first, nullptr);

  // Let the scheduler tick for a while without generating anything.
  std::this_thread::sleep_for(std::chrono::milliseconds(300));

  prosched::Process *second = sched.CreateNamedProcess("second", 256);
  ASSERT_NE(second, nullptr);

  const int gap = second->GetPID() - first->GetPID();
  sched.Stop();

  EXPECT_EQ(gap, 1)
      << "pid jumped from " << first->GetPID() << " to " << second->GetPID()
      << " (gap " << gap
      << ") with process generation switched off the entire time. "
         "GenerateProcessesCycle claims a pid every tick regardless of whether "
         "it creates anything, so pid numbers - and generated process names, "
